...
use metabase_api_rs::{ClientBuilder, api::Credentials};
use crate::integration::{get_metabase_url, get_test_email, get_test_password, wait_for_metabase};

#[tokio::test]
async fn test_email_password_authentication() {
    // Ensure Metabase is ready
    wait_for_metabase(30).await.expect("Metabase should be running");
...
async fn test_logout() {
    ...
    assert!(user_result.is_err(), "Should not be able to get user after logout");
}